...
SECTION H: ATTACHED EVIDENCE / SUPPORTING DOCUMENTS
-----------------------------------------------------
{chr(10).join(f"  {i+1}. {f}" for i, f in enumerate(data.get('evidence_files', []))) if data.get('evidence_files') else "  No supporting documents attached"}

SECTION I: COMPLAINANT'S REQUEST
---------------------------------
...
విభాగం H: జతచేసిన సాక్ష్యాలు / సహాయక పత్రాలు
---------------------------------------------------
{chr(10).join(f"  {i+1}. {f}" for i, f in enumerate(data.get('evidence_files', []))) if data.get('evidence_files') else "  ఎటువంటి సహాయక పత్రాలు జతచేయబడలేదు"}

విభాగం I: ఫిర్యాదుదారు అభ్యర్థన
------------------------------------
